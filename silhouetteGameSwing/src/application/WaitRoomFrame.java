  ...
  class ButtonListener implements ActionListener{
    ...
    public void actionPerformed(ActionEvent e) {
      ...
      if (b.getText().equals("방 생성하기")) {
        // 생성할 방의 제목을 입력받는다.
        roomName = JOptionPane.showInputDialog(null, "생성할 방 제목을 입력하세요.", "게임방 생성", JOptionPane.QUESTION_MESSAGE);
        
        if (roomName != null) { // roomName이 null이 아니면 서버에 "Tag//방이름" 형태로 메시지를 전송
          c.sendMsg(croomTag + "//" + roomName);
        } else { // null이면 생성 실패로 dialog 출력
          JOptionPane.showMessageDialog(null, "방 제목을 입력하셔야지 생성됩니다.", "게임방 생성 실패", JOptionPane.ERROR_MESSAGE);
      ...
      /* 방 입장하기 버튼 이벤트 */
        ...
        if(selRoom != null) { // selRoom이 null이 아니면 서버에 "Tag//방이름" 형태의 메시지를 전송
          ...
          JOptionPane.showMessageDialog(null, "입장할 방을 선택하셔야지 입장됩니다.", "게임방 입장실패", JOptionPane.ERROR_MESSAGE);
